module Web.Telegram.Types.Internal.LoginUrl where

import Common

-- | A parameter of the inline keyboard button used to automatically authorize
--   a user. Serves as a great replacement for the Telegram Login Widget when
--   the user is coming from Telegram. All the user needs to do is tap/click a
--   button and confirm that they want to log in:
--   ![a](https://core.telegram.org/file/811140015/1734/8VZFkwWXalM.97872/6127fa62d8a0bf2b3c)
data LoginUrl = LoginUrl
  { -- | An HTTP URL to be opened with user authorization data added to the
    ...
    --
    --   __NOTE:__ You must always check the hash of the received data to
    --   verify the authentication and the integrity of the data as described
    --   in [Checking authorization](https://core.telegram.org/widgets/login#checking-authorization).
    url :: Text,
